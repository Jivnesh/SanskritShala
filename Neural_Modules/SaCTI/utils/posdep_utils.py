        ...
        for token in sentence[TOKENS]:
            if type(token[ID]) == tuple and len(token[ID]) == 2:
                ...
                for word in token[EXPANDED]:
                    ...
                    new_ex[HEAD].append(edit_operation)
                    new_ex[DEPREL].append(edit_operation)
            else:
                src_text = token[TEXT]
        ...
        for token in sentence:
            if len(token[ID]) == 2:
                ...
                if get_vocab:
                    ...
                    headg = token.get(HEADG, 0)
                    deprelg = token.get(DEPRELG, '_')
                    # print("hehehe ",deprelg)
                    ...
                    vocabs[DEPRELG][deprelg] = vocabs[DEPRELG].get(deprelg, len(vocabs[DEPRELG]))
                    #vocabs[DEPRELG][deprelg] = vocabs[DEPRELG].get(edit_operation, len(vocabs[DEPRELG]))

                    # print(len(vocabs[DEPRELG]))
                ...
                new_ex[HEAD].append(head)
                new_ex[DEPREL].append(deprel)
                new_ex[HEADG].append(headg)
                new_ex[DEPRELG].append(deprelg)
                # new_ex[HEADG].append(edit_operation)
                # new_ex[DEPRELG].append(edit_operation)
