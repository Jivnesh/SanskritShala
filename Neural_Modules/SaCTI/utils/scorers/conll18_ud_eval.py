...
ID, FORM, LEMMA, UPOS, XPOS, FEATS, HEAD, DEPREL,HEADG, DEPRELG, DEPS, MISC = range(12)
...
def load_conllu(file):
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            self.is_content_deprelG = self.columns[DEPRELG] in CONTENT_DEPRELS
            self.is_functional_deprelG = self.columns[DEPRELG] in FUNCTIONAL_DEPRELS
    ...
    while True:
        ...
        columns = line.split("\t")
        if len(columns) != 12:
            raise UDError("The CoNLL-U line does not contain 12 tab-separated columns: '{}'".format(_encode(line)))
        ...
        if "-" in columns[ID]:
            ...
            for _ in range(start, end + 1):
                ...
                if len(word_columns) != 12:
                    raise UDError(
                        "The CoNLL-U line does not contain 12 tab-separated columns: '{}'".format(_encode(word_line)))
...
def evaluate(gold_ud, system_ud):
    ...
    return {
        ...
        "UASG": alignment_score(alignment, lambda w, ga: ga(w.parent)),
        "LASG": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPRELG])),
        "CLAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPRELG]),
