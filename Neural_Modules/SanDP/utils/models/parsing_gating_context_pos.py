...
class BiAffine_Parser_Gated(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, use_pos, use_char, pos_dim, num_pos, num_filters,
                 kernel_size, rnn_mode, hidden_size, num_layers, num_arcs, arc_space, arc_tag_space, num_gates,load_contextual_POS_paths,
        ...
        self.load_contextual_POS_paths = load_contextual_POS_paths
        self.rnn_encoder = BiRecurrentConv_Encoder(word_dim, num_words, char_dim, num_chars, use_pos, use_char,
                                                   ...
                                                   num_layers,load_contextual_POS_paths, embedd_word=embedd_word,
...
class BiRecurrentConv_Encoder(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, use_pos, use_char, pos_dim, num_pos, num_filters,
                 kernel_size, rnn_mode, hidden_size, num_layers,load_contextual_POS_paths, embedd_word=None, embedd_char=None, embedd_pos=None,
        ...
        self.context_pos_model = Sequence_Tagger(word_dim, num_words, char_dim, num_chars,
                            None, use_char, pos_dim, num_pos,
                            num_filters, kernel_size, rnn_mode,
                            hidden_size, num_layers, 200, 164)
        # self.context_pos_model.fc_3 = nn.Identity()
        print(self.context_pos_model)
        if use_pos:
    ...
    def forward(self, input_word, input_char, input_pos, mask=None, length=None, hx=None):
        ...
        if self.use_pos:
            if self.load_contextual_POS_paths:
                pos = self.context_pos_model(input_word, input_char, input_pos)[0]
