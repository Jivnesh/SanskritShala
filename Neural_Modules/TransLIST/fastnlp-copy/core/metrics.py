...
def _skt_tag_to_spans(tags, ignore_labels=None):
    """
    ________tags__lis______['O', 'B-singer', 'I-singer', 'I-singer', 'O', 'O']__
    ____[('singer', (1, 4))] (____________)

    :param tags: List[str],
    :param ignore_labels: List[str], ____list____label________
    :return: List[Tuple[str, List[int, int]]]. [(label__[start, end])]
    """
    ignore_labels = set(ignore_labels) if ignore_labels else set()
...
class SpanFPreRecMetric(MetricBase):
    ...
    def __init__(self, tag_vocab, pred=None, target=None, seq_len=None, encoding_type=None, ignore_labels=None,
        ...
        elif self.encoding_type == 'skt':
            self.tag_to_span_func = _skt_tag_to_spans
        else:
    ...
    def evaluate(self, pred, target, seq_len):
        ...
        for i in range(batch_size):
            ...
            pred_spans = ''.join(pred_str_tags).split('_')
            gold_spans = ''.join(gold_str_tags).split('_')

            for span in pred_spans:
                if span in gold_spans:
                    #self._true_positives[span[0]] += 1
                    self._true_positives[0] += 1
                    gold_spans.remove(span)
                else:
                    #self._false_positives[span[0]] += 1
                    self._false_positives[0] += 1
            for span in gold_spans:
                #self._false_negatives[span[0]] += 1
                self._false_negatives[0] += 1
